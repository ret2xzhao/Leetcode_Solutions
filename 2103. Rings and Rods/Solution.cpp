...
public:
    int countPoints(string rings) {
        ...
        for(int i=rings.size()-1; i>=1; i=i-2){
            colors[rings[i]-48].push_back(rings[i-1]);
        }
        for(auto str : colors){
            if(str.find('R')!=std::string::npos && str.find('G')!=std::string::npos && str.find('B')!=std::string::npos){
                ++count;
            }
        }
        return count;
